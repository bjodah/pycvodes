...
if [ $NATIVE -eq 1 -a ! -z $PYTHON ]; then
    >&2 echo "Cannot specify both --python and --native"
    exit 1
elif [ $NATIVE -eq 0 -a -z $PYTHON ]; then
    >&2 echo "Need to specify either --python or --native"
    exit 1
fi

SUNDBASE=$1
if [ ! -d "$SUNDBASE" ]; then >&2 echo "No such directory: $SUNDBASE"; exit 1; fi
...
if [ $TEST_ASAN -eq 1 ]; then
    ...
    export LDFLAGS="$LINKLIBS ${LDFLAGS:-} -fsanitize=address -Wl,-rpath,${LIBCXX_ASAN_ROOT}/lib -L${LIBCXX_ASAN_ROOT}/lib -lc++ -lc++abi -stdlib=libc++"
