...
if [[ "${LOW_PRECISION:-0}" != "1" ]]; then
    ...
    CXX=clang++-8 CC=clang-8 CFLAGS='-fsanitize=address -DPYCVODES_CLIP_TO_CONSTRAINTS=1' python3 setup.py build_ext -i
    PYTHONPATH=$(pwd) ASAN_OPTIONS=abort_on_error=1,detect_leaks=0 LD_PRELOAD=/usr/lib/llvm-8/lib/clang/8.0.1/lib/linux/libclang_rt.asan-x86_64.so ./scripts/run_tests.sh
