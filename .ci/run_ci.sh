...
cp -ra dist/ /tmp/
git clean -xfd
(cd /tmp/dist/; CC=gcc-10 CXX=g++-10 ${PYTHON:-python3} -m pip install $PKG_NAME-$PKG_VERSION.tar.gz)
...
if [[ "${LOW_PRECISION:-0}" != "1" ]]; then
    ...
    CXX=clang++-11 CC=clang-11 CFLAGS="-fsanitize=address -DPYCVODES_CLIP_TO_CONSTRAINTS=1 $CFLAGS" ${PYTHON:-python3} setup.py build_ext -i
    export PYTHON="env LD_PRELOAD=/usr/lib/llvm-11/lib/clang/11.0.1/lib/linux/libclang_rt.asan-x86_64.so ASAN_OPTIONS=abort_on_error=1,detect_leaks=0 ${PYTHON:-python3}" PYTHONPATH=$(pwd)
